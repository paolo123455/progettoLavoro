...
@Component({
  selector: 'app-prospetto-commessa',
  templateUrl: './prospetto-commessa.component.html',
  styleUrls: ['./prospetto-commessa.component.css']
})
export class ProspettoCommessaComponent {
  ...
  getRowId: GetRowIdFunc<any>  = params => params.data.anno + "_"+params.data.mese + "_" + params.data.nome_risorsa + params.data.cognome_risorsa;
...
this.insP.select(query).subscribe(response =>{console.log(response) ;
  ...
  var query = 
      `
      select distinct codice ,descrizione_progetto , codice_odl , descrizione_odl , ar.id_odl 
      from new_rilatt.attivita_risorsa ar 
      inner join new_rilatt.progetti p  on p.id_progetto  =  ar.id_progetto 
      left  join new_rilatt.odl p2 
      on p2.id_odl = ar.id_odl
      order by descrizione_progetto 
      `
  ...
  elaborazioneStruttura  = (json : Object) =>{
    ...
    var query = `      select distinct  ar.cognome as cognome_risorsa , ar.nome  as nome_risorsa , r.anno , r.mese  `
    var query2 =      `from new_rilatt.progetti p2
                       inner join new_rilatt.attivita_risorsa r  on p2.id_progetto = r.id_progetto  
                       inner join new_rilatt.risorse ar  on ar.id_risorsa  = r.id_risorsa 
                       left join new_rilatt.odl  o on  o.id_odl = r.id_odl
                       group by  ar.cognome , ar.nome  , r.anno , r.mese 
                       order by cognome `
    ...
    for( let element of listaR ) {
               ...
               casewhen = `
                          , Min(
                                CASE  WHEN p2.descrizione_progetto = '` + element.descrizione_progetto+ `'`+
                          `     AND  p2.codice = '` + element.codice+ `'`+
                          `     AND  (o.descrizione_odl is null OR o.descrizione_odl = '` + element.descrizione_odl+ `')`+
                          `     AND  (o.codice_odl is null OR o.codice_odl = '` + element.codice_odl + `')`+
                          ` THEN giornate End) as  "`+element.descrizione_progetto+"-"+ element.codice  + "-"+element.descrizione_odl +"-"+ element.codice_odl + `",
                            Min(
                                CASE  WHEN p2.descrizione_progetto = '` + element.descrizione_progetto+ `'`+
                          `     AND  p2.codice = '`+ element.codice+ `'`+
                          `     AND  (o.descrizione_odl is null OR o.descrizione_odl = '` + element.descrizione_odl+ `')`+
                          `     AND  (o.codice_odl is null OR o.codice_odl = '` + element.codice_odl+ `')`+
                          ` THEN flag_attivita || '' End) as  "flag_`+element.descrizione_progetto+"-"+ element.codice  + "-"+element.descrizione_odl +"-"+ element.codice_odl + `"`
                          
                query = query + casewhen
               this.columnDefs.push({ 
                field : element.descrizione_progetto+"-"+ element.codice  + "-"+element.descrizione_odl +"-"+ element.codice_odl, 
                headerName: element.descrizione_progetto +"-"+ element.codice  + "\n" + element.descrizione_odl +"-"+ element.codice_odl,
                headerTooltip: element.codice + "\n" + element.codice_odl ,   cellStyle: params => {
                      if (params.data["flag_"+element.descrizione_progetto]  +""== 'true') {
                          return {color: 'black', backgroundColor: '#FFCCCB'}
                         }
                      return {color: 'black', backgroundColor: "deafault"}},
                editable: (params) => !params.data["flag_"+element.descrizione_progetto+ "-" + element.descrizione_odl] })
    };
    
    console.log(query +query2)
    this.insP.select(query+query2).subscribe(response=>{
